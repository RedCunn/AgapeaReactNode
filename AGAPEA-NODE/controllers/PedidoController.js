...
function _generarJWT( { nombre,apellidos, email, idcliente }, expires='1h' ){
    return  jsonwebtoken.sign(
        ...
        { expiresIn: expires, issuer: 'http://localhost:3003' } //opciones o lista de claims predefinidos
    );
}

async function _returnClientData(idcliente){
    try {
...
module.exports={
    completeOrder: async (req,res,next)=>{
        try {
            ...
            //en req.body van los datos de la pet.ajax del request: { datosEnvio, datosFactura, datosPago, elementosPedido, gastosEnvio, clienteLogged}
            console.log('body de la peticion...', req.body);
            let { datosEnvio, datosFactura, datosPago, elementosPedido, gastosEnvio, clienteLogged }=req.body;
            ...
            if (datosPago.pagoradios==='pagotarjeta') {
                ...
                if (await stripeservice.createCharge(_customerStripeId, _cardId, _totalPedido, _idnuevoPedido)) {
                    ...
                    let _cliente=await _returnClientData(clienteLogged.datoscliente._id);
    ...
    paypalCallBack: async (req,res,next)=>{ //OJO!!!!! ESTE METODO ES INVOCADO POR EL SERVER DE PAYPAL, NO EL CLIENTE DE REACT!!!!
            ...
            let _paypalpayment=await PayPalPayment.findOne( { idcliente: idcli, idpedido: pedid } );
            console.log('valor del coleccion PayPalPayment para recuperar idpago...', _paypalpayment);
            
            let _finPagoOK=await paypalservice.finalizarPagoPayPal(_paypalpayment.idpago);
            ...
            //completeOrderOK quien esta realmente pidiendo los datos del cliente asociados al id q hay en la url y NO ES UN ATACANTE q intenta
            //obtener datos por inyeccion al endpoint
            let _jwtSoloUnUso=jsonwebtoken.sign( 
                    { idcliente: idcli, idpedido: pedid, idpago: _paypalpayment.idpago },
            ...
            res.status(200).redirect(`http://localhost:3000/Pedido/completeOrderOK?idpedido=${pedid}&idcliente=${idcli}&token=${_jwtSoloUnUso}`);
    }
}